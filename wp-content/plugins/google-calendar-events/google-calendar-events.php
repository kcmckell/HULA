...
if ( ! class_exists( 'Google_Calendar_Events' ) ) {
	class Google_Calendar_Events {
		...
		function add_styles() {
			wp_enqueue_style( 'gce_styles', plugins_url( GCE_TEXT_DOMAIN.'/css/gce-style.css', basename(dirname(__FILE__ )) ));

			$options = get_option( GCE_GENERAL_OPTIONS_NAME );

			//If old stylesheet option is enabled, enqueue old styles
			if ( $options['old_stylesheet'] )
				wp_enqueue_style( 'gce_old_styles', plugins_url( GCE_TEXT_DOMAIN.'/css/gce-old-style.css', basename(dirname(__FILE__ )) ));
		...
		function add_scripts() {
			...
			wp_enqueue_script( 'gce_jquery_qtip', plugins_url( GCE_TEXT_DOMAIN.'/js/jquery-qtip.js', basename(dirname(__FILE__ ))), array( 'jquery' ), null, $add_to_footer );
			wp_enqueue_script( 'gce_scripts', plugins_url( GCE_TEXT_DOMAIN.'/js/gce-script.js', basename(dirname(__FILE__ ))), array( 'jquery' ), null, $add_to_footer );
