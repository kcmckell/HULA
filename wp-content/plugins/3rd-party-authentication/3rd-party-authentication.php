...
if (! class_exists('ThirdPartyPlugin')) {
	...
	class ThirdPartyPlugin {
		function ThirdPartyPlugin() {
			...
			if (!(bool) get_site_option('3rd_party_allow_regular')) {
		...
		function initialize_options() {
			if ($this->is_admin()) {
				add_site_option('3rd_party_allow_regular', false, 'Allow regular logins as well as email logins?');
				add_site_option('3rd_party_google_apps_dont', false, "Don't authenticate gmail/googlemail logins?");
				add_site_option('3rd_party_google_apps_all', false, 'Authenticate all domains via google apps?');
				add_site_option('3rd_party_google_apps_create', false, 'Automatically create users that don\'t exist?');
				add_site_option('3rd_party_google_apps_domains', '', 'A comma seperated list of domains to authenticate via google apps.');
			}
		}
		...
		function add_options_page() {
			if (is_multisite()) {
				add_submenu_page('settings.php', '3rd Party Authentication', '3rd Party Authentication', 'manage_options', '3rd_party_auth', array(&$this, '_display_options_page'));
			} else {
				add_options_page('3rd Party Authentication', '3rd Party Authentication', 9, '3rd_party_auth', array(&$this, '_display_options_page'));
			}
		}
		
		function google_domains() {
			$domain_option = get_site_option('3rd_party_google_apps_domains');
			if (isset($domain_option) && trim($domain_option) != '') {
				$domains = explode(",",ereg_replace(' ','',$domain_option));
			}
			if (!(bool) get_site_option('3rd_party_google_apps_dont')) {
					$domains[] = 'gmail.com';
		...
		function domain_list() {
			...
			$email_settings = get_site_option('3rd_party_email_settings');
		...
		function login_failed($username) {
			...
			$create_users = (bool) get_site_option('3rd_party_google_apps_create');
		...
		function use_google($domain) {
			$googleall = (bool) get_site_option('3rd_party_google_apps_all');
			if (!$googleall) {
		...
		function check_password($check, $password, $hash, $user_id) {
			...
			if ($check && ((bool) get_site_option('3rd_party_allow_regular') || ($username == 'admin' && $user->user_level >= 10))) {
				return true;
			} else {
				$parts = explode("@",$username);
				if (count($parts) != 2) {
		...
		function authenticate($username, $password) {
			...
			if ($username != '' && $google->Authenticate($username,$password)) {
				...
				if (! $user or $user->user_login != $username) {
					if ((bool) get_site_option('3rd_party_auto_create_user')) {
		...
		function _display_options_page() {
			...
			if (isset($submit)) {
				if (is_multisite()) {
					update_site_option('3rd_party_allow_regular', $_REQUEST['3rd_party_allow_regular']);
					update_site_option('3rd_party_google_apps_dont', $_REQUEST['3rd_party_google_apps_dont']);
					update_site_option('3rd_party_google_apps_all', $_REQUEST['3rd_party_google_apps_all']);
					update_site_option('3rd_party_google_apps_create', $_REQUEST['3rd_party_google_apps_create']);
					update_site_option('3rd_party_google_apps_domains', $_REQUEST['3rd_party_google_apps_domains']);
				}
			}
			
			$submit = $_REQUEST['SubmitEmail'];
			if (isset($submitemail)) {
				...
				update_site_option('3rd_party_email_settings',$new_email_settings);
			}
			
			$allow_regular = (bool) get_site_option('3rd_party_allow_regular');
			$create_users = (bool) get_site_option('3rd_party_google_apps_create');
			$google_apps_dont = (bool) get_site_option('3rd_party_google_apps_dont');
			$google_apps_all = (bool) get_site_option('3rd_party_google_apps_all');
			$google_apps_domains = get_site_option('3rd_party_google_apps_domains');
			$email_settings = get_site_option('3rd_party_email_settings');
?>
<div class="wrap">
  <h2>3rd Party Authentication Options</h2>
  <form action="<?php echo is_multisite() ? '' : 'options.php'?>" method="post">
...
?>
    ...
    <p class="submit">
	<input type="submit" name="SubmitEmail" value="Save Email Settings" />
	</p>
	</form>
</div>
<?php
